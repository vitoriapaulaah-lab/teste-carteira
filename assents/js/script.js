...
function setFilter(status) {
  ...
  function normalizarChaveObraResumo(value) {
  ...
  function getObraKeyResumo(row) {
    ...
    const metaKey = normalizarChaveObraResumo(detalhes.meta_obra_key);
    if (metaKey) return metaKey;
    return normalizarChaveObraResumo(row && row[COLS.OBRA]);
  }
  ...
  function expandirLinhasCarteiraPorPedido(dados) {
    return (Array.isArray(dados) ? dados : []).flatMap(item => {
      ...
      const detalhes = safeJsonParse(row[COLS.DETALHES_JSON], {});
      const linhasCarteira = Array.isArray(detalhes.meta_carteira_linhas) ? detalhes.meta_carteira_linhas : [];

      if (!linhasCarteira.length) return [item];
      ...
      return linhasCarteira
        ...
        .map((linha, idx) => ({
          content: linha,
          originalIndex: item.originalIndex,
          renderKey: `${item.originalIndex}:carteira:${idx}`
        }));
    });
  }

  function contarObrasUnicasResumo(dados) {
    const chaves = new Set();

    (Array.isArray(dados) ? dados : []).forEach(item => {
      const row = Array.isArray(item && item.content) ? item.content : [];
      const key = getObraKeyResumo(row);
  ...
  function renderizar(dadosOriginais) {
    ...
    const dadosBaseStatus = currentStatusFilter === 'FIRMADAS'
      ? expandirLinhasCarteiraPorPedido(dadosOriginais)
      : dadosOriginais;

    const dadosFiltradosStatus = dadosBaseStatus.filter(d => {
    ...
    const qtdObrasResumo = currentStatusFilter === 'FIRMADAS'
      ? contarObrasUnicasResumo(dadosOrdenados)
      : dados.length;
