...
function setFilter(status) {
  ...
  function calcularDataLimitePrazoCarteira(r) {
    const row = Array.isArray(r?.content) ? r.content : r;
    if (!row) return null;

    const dataAbertura = normalizarDataZeroHora(parseDataUniversal(row[COLS.DATA_ABERTURA]));
    if (!dataAbertura) return null;

    const prazoRaw = String(row[COLS.DIAS_PRAZO] ?? "").trim();
    if (!prazoRaw || prazoRaw === "-" || prazoRaw.toUpperCase() === "N/A") return null;

    if (isStatusDate(prazoRaw) || isIsoDate(prazoRaw)) {
      return normalizarDataZeroHora(parseDataUniversal(prazoRaw));
    }

    const diasPrazo = parseFloat(
      prazoRaw
        .replace(/\s/g, "")
        .replace(",", ".")
        .replace(/[^\d.-]/g, "")
    ...
    if (!Number.isFinite(diasPrazo) || diasPrazo < 0) return null;

    return addDiasCorridos(dataAbertura, Math.round(diasPrazo));
  }

  function calcularStatusPrazoCarteira(r) {
    const limite = calcularDataLimitePrazoCarteira(r);

    if (!limite) {
      return { texto: "-", valor: 0, atrasoDias: 0, atraso: false, diasRestantes: null, classe: "days-info" };
    }

    const hoje = normalizarDataZeroHora(new Date());
    const dtLimite = normalizarDataZeroHora(limite);
    ...
    const utcHoje = Date.UTC(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
    const utcLimite = Date.UTC(dtLimite.getFullYear(), dtLimite.getMonth(), dtLimite.getDate());
    const diffDias = Math.round((utcLimite - utcHoje) / 86400000);

    if (diffDias < 0) {
      const atrasoDias = Math.abs(diffDias);
      return {
        texto: `${atrasoDias}d atraso`,
        valor: 100000 + atrasoDias,
        atrasoDias,
        atraso: true,
    ...
    return {
      texto: `Faltam ${diffDias}d`,
      valor: diffDias <= 5 ? 80000 - diffDias : Math.max(1, 50000 - diffDias),
      atrasoDias: 0,
      atraso: false,
      diasRestantes: diffDias,
      classe: diffDias <= 5 ? "days-warning" : "days-ok"
    };
  }
  ...
  function ordenarDados(dados) {
    ...
    return dados.slice().sort((itemA, itemB) => {
      ...
      if (chave === 'obra') { valorA = String(rA[COLS.OBRA] || '').trim(); valorB = String(rB[COLS.OBRA] || '').trim(); } 
        ...
        const pA = isFiltroCarteiraAtual() ? calcularStatusPrazoCarteira(rA) : calcularPorcentagem(rA);
        const pB = isFiltroCarteiraAtual() ? calcularStatusPrazoCarteira(rB) : calcularPorcentagem(rB);
        valorA = pA.valor;
        valorB = pB.valor;
      } 
  ...
  function renderizar(dadosOriginais) {
    ...
    if (!isGeralView) {
      ...
      const labs = ["OBRA", "VALOR", "PEDIDO", "CLIENTE", "ITEM", "STATUS DO PRAZO", "STATUS DE COMPRAS", ...ITENS, "OBSERVAÇÕES"];
      ...
      dadosOrdenados.forEach((dO, renderIdx) => {
        ...
        const res = calcularStatusPrazoCarteira(r);
        ...
        html += `<td><span class="days-badge ${res.classe || (res.atraso ? "days-urgent" : "days-ok")} shadow-sm">${res.texto}</span></td>`;
        ...
        <div class="mc-card animate-fade-up" onclick="abrirLinhaRenderizada(${renderIdx})">
            <div class="mc-header">
                ...
                <span class="days-badge ${res.classe || (res.atraso ? "days-urgent" : "days-ok")} shadow-sm">${res.texto}</span>
            </div>
            <div class="mc-body">
