const ITENS = ["BBA/ELET.", "MT", "FLUT.", "M FV.", "AD. FLEX", "AD. RIG.", "FIXADORES", "SIST. ELÉT.", "PEÇAS REP.", "SERV.", "MONT.", "FATUR."];
  ...
  function getNumeroPedidoDisplay(row) {
    ...
    candidatos.forEach(valor => {
      pedidos = pedidos.concat(normalizarListaPedidoDisplay(valor));
    });
  ...
  function renderPedidoBadge(row) {
    ...
    if (!pedido) return `<span class="pedido-badge pedido-badge-empty">-</span>`;
    return `<span class="pedido-badge" title="${escapeHtml(pedido)}">#${escapeHtml(pedido)}</span>`;
  }
  ...
  function renderizar(dadosOriginais) {
    ...
    if (isFiltroCarteiraAtual()) {
      ...
      if (currentStatusFilter === 'TODAS') {
        ...
        dadosPreparados = isFiltroEntregueAtual()
